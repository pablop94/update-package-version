...
function show_help(currentVersion) {
  ...
  console.log(`npx modify-version ${currentVersion}`, "\x1b[32m", "Updates the project's version to the passed one.", "\x1b[36m")
  console.log('npx modify-version patch', "\x1b[32m", 'Increments one the patch version (x.y.z+1).', "\x1b[36m")
  console.log('npx modify-version minor', "\x1b[32m", 'Increments one the minor version (x.y+1.z).', "\x1b[36m")
  console.log('npx modify-version major', "\x1b[32m", 'Increments one the major version (x+1.y.z).', "\x1b[36m")
  console.log('npx modify-version current', "\x1b[32m", 'Returns the current version number from package.json file.', "\x1b[36m")
  console.log('npx modify-version config', "\x1b[32m", 'Configures the file needed to work, you can edit it with your own configuration.', "\x1b[36m")
}
...
function update_version(new_version) {
  ...
  try {
    const hooks = JSON.parse(fs.readFileSync(path.join(cwd, '/modify-version.json')).toString())
    hooks.forEach(hook => mappings[hook.type](...hook.params)(currentVersion, new_version))
  }
  catch(e){
    throw new Error('Not found configuration file: modify-version.json. Run npx modify-version config to create one.')
  }
}
